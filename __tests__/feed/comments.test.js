...
describe("Feed All Comments Pagination", () =>{
    let testUser;
    let authToken;
    ...
    it("should return 404 when no comments exist on feed", async ()=>{
        const response = await request(app)
            .get(`/api/v1/feed/${testUser.username}/comments`)
            .set("Cookie", [`token=${authToken}`]);

        expect(response.status).toBe(404);
    });

    it("should return paginated comments for feed", async ()=>{
