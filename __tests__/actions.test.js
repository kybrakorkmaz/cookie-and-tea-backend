...
import {createAction, fetchActionsForUser, deleteExpiredReadActions} from "../src/repositories/actions.repository.js";
...
describe("Actions (Notifications) Integration", () =>{
    ...
    it("should mark action as read and purge old reads", async ()=>{
        // create a read action with old readAt (15 days ago)
        ...
        // ensure old read actions removed
        const actionsAfter = await fetchActionsForUser(userB.id);
        const ids = actionsAfter.map(a => a.message);
        expect(ids).not.toContain('old comment');
    });
});
